<%@page import="com.jxhc.tenpay.config.TenpayConfig"%>
<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
<%@ page import="com.jxhc.tenpay.util.TenpayUtil" %>
<%@ page import="com.jxhc.tenpay.RequestHandler"%>
...
//获取提交的商品价格
String order_price=new String(request.getAttribute("WIDtotal_fee").toString().getBytes("ISO-8859-1"),"UTF-8");
/* 商品价格（包含运费），以分为单位 */
double total_fee = (Double.valueOf(order_price) * 100);
int fee = (int)total_fee;
  
//获取提交的商品名称
String product_name=new String(request.getAttribute("WIDsubject").toString());//订单名称
System.out.println("product_name"+product_name);
String desc = new String(request.getAttribute("WIDbody").toString());
...
//服务器异步通知页面路径
String notify_url = "http://" + request.getServerName() + ":" + request.getLocalPort() + request.getContextPath() + TenpayConfig.notify_url;
//需http://格式的完整路径，不能加?id=123这类自定义参数
System.out.println("product_name"+notify_url);
//页面跳转同步通知页面路径
String return_url = "http://" + request.getServerName() + ":" + request.getLocalPort() + request.getContextPath() + TenpayConfig.return_url;
